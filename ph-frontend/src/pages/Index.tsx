...
const Index = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      setProjects(allProjects); // Set all projects in state

      // Filter user-specific projects based on logged-in user's ID
      const userId = localStorage.getItem("userId"); // Ensure userId is stored during login
      const userSpecificProjects = allProjects.filter(
        (project) => project.creatorId === parseInt(userId)
      );
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      axios
        ...
        .then(() => {
          setIsAuthenticated(true);
          fetchProjects(); // Fetch projects once authentication is verified
        })
        .catch(() => {
