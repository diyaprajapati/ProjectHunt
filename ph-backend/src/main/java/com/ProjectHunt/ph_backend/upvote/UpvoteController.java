...
//            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred while toggling upvote");
//        }
//    }
@PostMapping("/{projectId}")
public ResponseEntity<Map<String, Object>> toggleUpvote(@PathVariable Long projectId, @AuthenticationPrincipal User user) {
    boolean upvoted = upvoteService.toggleUpvote(projectId, user);
